...
export default function Main() {
  ...
  const [types, setTypes] = useState({
    ...
    enchantment: {
      name: 'Enchantments',
      url: '/images/enchantment.png',
      status: true
    },
    instant: {
      name: 'Instants',
      url: '/images/instant.png',
      status: true
    },
    land: {
      name: 'Lands',
  ...
  const calculateStats = () => {
    ...
    deck.deckArray.forEach(async (item) => {
      const res = await fetch(
        'https://api.scryfall.com/cards/named?fuzzy=' + item
      )
  ...
  const setCommanderImage = () => {
    deck.commander.forEach((item) => {
      fetch('https://api.scryfall.com/cards/named?fuzzy=' + item)
        ...
        .then((json) => {
          setUrls((prevUrls) => {
            return [...prevUrls, json.image_uris.normal]
          })
        })
    })
  }
  ...
  const redoStats = () => {
    ...
    if (cmcMods.current.cmd) {
      ...
      firstStat.current && currentManaSymbols.shift()
      firstStat.current = false
    }
  ...
  const loadCardModalData = async (cardName) => {
    ...
    const cardLegality =
      json.legalities.commander.charAt(0).toUpperCase() +
      json.legalities.commander.slice(1)
    setCardInfo({
    ...
    <div className='mainwrap'>
      {/* <div className='topbar'>
        <div className='stuff-to-hide' hidden={visibilities.featuredDecks}>
          ...
          <textarea
            className='paste-deck'
            placeholder='Paste a deck here'></textarea>
        ...
        <button
          hidden={visibilities.gameButton}
          className='ec-button'
        ...
        <div className='stats-container' hidden={visibilities.statsText}>
          <div className='text-stats'>
            <span>{`Average CMC: ${stats.avgCMC}`}</span>
          </div>
          <div className='cmc-settings'>
            <input
              type='checkbox'
              className='check-stats'
              onChange={(e) => {
                cmcMods.current.cmd = e.target.checked
                redoStats()
              }}
            />
            <label>Include Commander</label>
            <br />
            <input
              type='checkbox'
              className='check-stats'
              onChange={(e) => {
                cmcMods.current.phyrexian = e.target.checked
                redoStats()
              }}
            />
            <label>Count Phyrexian Mana as 0</label>
            <br />
            <input
              type='checkbox'
              className='check-stats'
              onChange={(e) => {
                cmcMods.current.lands = e.target.checked
                redoStats()
              }}
            />
            <label>Include Lands in CMC</label>
          </div>
        ...
        <div className='filter-grid' hidden={visibilities.deckFilters}>
          {Object.keys(types).map((key, index) => {
            let fclass
            !types[key].status
              ? (fclass = 'filter-item filter-gray')
              : (fclass = 'filter-item')
            return (
              <div
        ...
        <div
          className='preview-on-hover'
          hidden={visibilities.previewCardOnHover}>
          <img
            alt='Hover over a card to view'
            width='200'
      ...
      <div className='tournament-bar'>
        <h5 className='tournament-text'>Open tournaments starting soon</h5>
        <div className='tournament'>
          <span className='tournament-name'>OP tournament</span>
          <span className='tournament-creator'>Created by matyi</span>
          <span className='players'>55 out of 64 players</span>
          <span className='starting'>Starting at 8:00 (in 36 minutes)</span>
        </div>
        <div className='tournament'>
          <span className='tournament-name'>Greg's Birthday Tourney</span>
          <span className='tournament-creator'>Created by gregorious4</span>
          <span className='players'>55 out of 64 players</span>
          <span className='starting'>Starting at 8:00 (in 36 minutes)</span>
        </div>
      </div>
      <h3>Popular Decks</h3>
      <div className='decks-line'></div>
      {!visibilities.featuredDecks && <FeaturedDeck />}
      <div className='loading-bar' hidden={visibilities.loadingBar}>
        <div
          className='loaded'
          style={{
            width: `${(loading.length / deck.deckArray.length) * 100}%`
          }}></div>
      </div>
        ...
        {/* eslint-disable-next-line */}
        {
          deck.deckArray.map((item, index) => {
            ...
            Object.keys(types).forEach((key) => {
              !types[key].status &&
                cardTypes.get(item).indexOf(types[key].name.slice(0, -1)) !==
                  -1 &&
                (dontRender = true)
            })
            ...
            if (!dontRender)
              return (
                <div
                  key={index}
                  onMouseEnter={() => previewCard(item)}
                  onClick={() => loadCardModalData(item)}
                  className='card'>
                  {item}
                </div>
              )
          ...
          deck.basics.map((item, index) => {
            ...
            if (item !== 0) {
              return (
                <div
                  key={`pr${index}`}
                  onMouseEnter={() => previewCard(basicsList[index])}
                  onClick={() => loadCardModalData(basicsList[index])}
                  className='card'>{`${item} ${basicsList[index]}`}</div>
              )
            }
          })}
      </div>
            ...
            <React.Fragment key={key}>
              ...
              <animated.div style={props} className='modal-main'>
                  ...
                  <form
                    ...
                    <div class='input'>
                      <input
                        ...
                        placeholder=' '
                      />
                      <label className='input__label'>Game ID</label>
                    </div>
                    <div class='input'>
                      <input
                        name='username'
                        ...
                        placeholder=' '
                      />
                      <label className='input__label'>Nickname</label>
                    </div>
                  </form>
                </div>
                <div className='modal-bottom'>
                  <div
                    className='ec-button'
                    onClick={() => gameForm.current.submit()}>
                    Join Game
                  </div>
                  <div
      ...
      {cardModalTransition.map(
        ({ item, key, props }) =>
          !item && (
            <React.Fragment key={`cm${key}`}>
              <div
                className='modal-shade'
                hidden={visibilities.cardModal}
                onClick={() =>
                  setVisibilities({ ...visibilities, cardModal: true })
                }></div>
              <animated.div
                style={props}
                className='modal-main modal-main-card'>
